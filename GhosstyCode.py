...
# =================================================================
# 💎 SECTION 11: PRODUCT CARD & SMART EMOJI (PREMIUM UI)
# =================================================================

async def show_product_card(update: Update, context: ContextTypes.DEFAULT_TYPE, product_id: int):
    """
    Генерує преміальну картку товару. 
    Викликається, коли юзер натискає на товар у списку (callback view_item_).
    """
    query = update.callback_query
    user_id = update.effective_user.id
    profile = context.user_data.get('profile', {})
    
    # Використовуємо твою функцію з Section 3 для пошуку товару
    product = get_item_data(product_id)

    if not product:
        await query.answer("⚠️ Товар тимчасово недоступний", show_alert=True)
        return

    # Розрахунок ціни через твій calculate_final_price (Section 4)
    item_price = product.get('price', 0)
    final_price, is_discounted = calculate_final_price(item_price, profile, item_id=product_id)
    
    stock = product.get('stock', 0)
    stock_status = "✅ В наявності" if stock > 0 else "❌ Очікується"
    vip_badge = "💎 VIP" if profile.get('is_vip') else "🔘 BASIC"
    
    # Формування ціни (закреслена, якщо є знижка)
    price_text = f"💰 Ціна: <b>{int(final_price)} грн</b>"
    if is_discounted:
        price_text = f"💰 Ціна: <s>{int(item_price)} грн</s> → <b>{int(final_price)} грн</b> 💎"

    caption = (
        f"📦 <b>{product['name'].upper()}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{product.get('desc', 'Опис відсутній')}\n\n"
        f"📊 Статус: {stock_status} ({stock} шт.)\n"
        f"👤 Ваш статус: <b>{vip_badge}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{price_text}\n"
        f"🚚 Доставка: Дніпро (Кур'єр/Клад) | НП"
    )

    kb = []
    if stock > 0:
        kb.append([InlineKeyboardButton("🛒 Додати в кошик", callback_data=f"cart_add_{product_id}")])
        kb.append([InlineKeyboardButton("⚡️ Купити в 1 клік", callback_data=f"fast_buy_{product_id}")])
    
    # Повернення до категорії (беремо тип товару для коректного callback)
    cat_type = product.get('type', 'all')
    kb.append([InlineKeyboardButton("🔙 Назад до списку", callback_data=f"cat_list_{cat_type}")])

    # Отримання фото (твоє поле 'img')
    image_url = product.get('img', "https://i.ibb.co/placeholder.png")
    
    # Використовуємо твій UI Engine для оновлення
    await _edit_or_reply(query, caption, kb, photo=image_url, context=context)
    

# =================================================================
# 🌍 SECTION 10: GEOGRAPHY & LOGISTICS (TITAN SYNC FIX)
# =================================================================

async def choose_city_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    КРОК 1: Глобальне меню вибору локації.
    Очищає старі стани та готує профіль до оновлення.
    """
    query = update.callback_query
    if query: await query.answer()

    # Скидаємо стани, щоб уникнути конфліктів при зміні локації
    ...
    text = (
        ...
        "<i>*Для інших міст обирайте 'Нова Пошта'</i>"
    )
    
    # Пріоритетні міста + Нова Пошта
    cities_db = globals().get('UKRAINE_CITIES', {})
    if not cities_db:
        cities_db = {"Дніпро": [], "Кам'янське": [], "Київ": [], "Львів": [], "Одеса": []}
    
    city_list = list(cities_db.keys())
    keyboard = []

    # Спеціальна кнопка для Дніпра (через хаб)
    keyboard.append([InlineKeyboardButton("📍 Дніпро (Всі райони + Кур'єр)", callback_data="hub_dnipro")])
    
    # Інші міста по 2 в ряд
    other_cities = [c for c in city_list if c != "Дніпро"]
    for i in range(0, len(other_cities), 2):
        row = [InlineKeyboardButton(other_cities[i], callback_data=f"sel_city_{other_cities[i]}")]
        if i + 1 < len(other_cities):
            row.append(InlineKeyboardButton(other_cities[i+1], callback_data=f"sel_city_{other_cities[i+1]}"))
        keyboard.append(row)
    
    # Глобальна доставка
    keyboard.append([InlineKeyboardButton("🚚 Нова Пошта (Вся Україна)", callback_data="sel_city_NP")])
...
async def choose_dnipro_delivery(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Миттєво фіксує місто в БД при вході в меню.
    """
    query = update.callback_query
    await query.answer()
    ...
    # ФІКС: Оновлюємо місто відразу при переході в хаб
    try:
        with sqlite3.connect(DB_PATH, timeout=30) as conn:
            conn.execute("UPDATE users SET city='Дніпро', district=NULL WHERE user_id=?", (user_id,))
            conn.commit()
        context.user_data.setdefault("profile", {})["city"] = "Дніпро"
        context.user_data["profile"]["district"] = None
    except Exception as e:
        logger.error(f"Sync error in Dnipro hub: {e}")
    
    text = (
        "🏙 <b>ДНІПРО: СПОСІБ ОТРИМАННЯ</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "📦 <b>Район (Клад)</b> — забирай готовий сховок самостійно.\n"
        "🛵 <b>Кур'єр (+150 грн)</b> — доставка прямо до дверей.\n\n"
        "👇 Що обираєте?"
    )
...
async def district_selection_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, city: str):
    """
    КРОК 2: Динамічне меню районів з повною синхронізацією бази.
    """
    query = update.callback_query
    user_id = update.effective_user.id
    
    # Визначаємо реальну назву міста для БД
    if city == "Dnipro_Klad": real_city = "Дніпро"
    elif city == "NP": real_city = "Нова Пошта"
    else: real_city = city
        
    # СИНХРОНІЗАЦІЯ БД + КОНТЕКСТ
    try:
        with sqlite3.connect(DB_PATH, timeout=30) as conn:
            conn.execute("UPDATE users SET city=?, district=NULL WHERE user_id=?", (real_city, user_id))
            conn.commit()
        
        context.user_data.setdefault('profile', {})['city'] = real_city
        context.user_data['profile']['district'] = None
    except Exception as e:
        logger.error(f"Critical Sync Error (District Handler): {e}")

    # Якщо обрана Нова Пошта - одразу на введення даних
    if real_city == "Нова Пошта":
        await ask_delivery_details(update, context)
        return

    cities_db = globals().get('UKRAINE_CITIES', {})
    districts = cities_db.get(real_city, [])
    
    # Дефолтні райони для Дніпра (якщо база пуста)
    if real_city == "Дніпро" and not districts:
        districts = ["Центр", "Соборний", "Шевченківський", "Амур", "Індустріальний", "Самарський", "Чечелівський", "Новокодацький"]

    text = (
        f"🏘 <b>{real_city.upper()}: ОБЕРІТЬ РАЙОН</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"Оберіть район, де вам зручно отримати замовлення:"
    )
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 ВІДРЕФАКТОРЕНО: Справжній динамічний VIP статус
    is_vip = profile.get('is_vip', False)
    vip_text = "💎 <b>VIP PRO</b>" if is_vip else "👤 <b>Standard</b>"
    discount_text = "<b>-35%</b> (VIP)" if is_vip else "<b>0%</b> (Standard)"
    delivery_text = "<b>БЕЗКОШТОВНА</b> (VIP)" if is_vip else "<b>За тарифами НП/Кур'єра</b>"
    ...
    welcome_text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Йо, <b>{safe_name}</b>! Твій статус: {vip_text}\n\n"
        f"💰 Твій баланс: <b>{current_balance} грн</b>\n"
        f"📉 Знижка: {discount_text}\n"
...
# =================================================================
# 🛒 SECTION 18: CART LOGIC (TITAN ULTIMATE v10.5 - PRO SYNC)
# =================================================================
...
async def show_cart_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    🔥 ВДОСКОНАЛЕННЯ: Повна синхронізація з профілем та базою даних.
    """
    ...
    # Отримуємо актуальний профіль (з пам'яті або БД)
    profile = context.user_data.get("profile", {})
    user_id = update.effective_user.id
    ...
    for item in cart:
        item_id = item.get('real_id') # ID товару з бази
        
        # Використовуємо твою функцію get_price_display (Section 4)
        ...
        # Формуємо деталі (колір/подарунок)
        details = []
        ...
        # Кнопка видалення конкретного екземпляра (по його унікальному id у кошику)
        uid = item.get('id', 0)
        keyboard.append([InlineKeyboardButton(f"❌ Видалити: {name[:18]}", callback_data=f"cart_del_{uid}")])

    # --- ПЕРЕВІРКА ДАНИХ ДОСТАВКИ (Синхрон з БД) ---
    # Перевіряємо чи є всі дані в профілі
    full_name = profile.get("full_name")
    phone = profile.get("phone")
    city = profile.get("city")
    address = profile.get("address_details")
    
    can_checkout = all([full_name, phone, city and city != "Не обрано", address])
    ...
    gift_announcement = ""
    if has_gift_in_cart:
        gift_announcement = "🎉 <i>У вашому замовленні є бонус!</i>\n━━━━━━━━━━━━━━━━━━━━\n"
    ...
    full_text = (
        ...
        f"{gift_announcement}"
        f"{loc_status}\n"
        f"💰 <b>РАЗОМ ДО СПЛАТИ: {int(total_sum)} UAH</b>"
    )
    ...
    footer = []
    # Якщо ще немає знижки на наступне замовлення, пропонуємо промокод
    ...
    if update.callback_query:
        await _edit_or_reply(update.callback_query, full_text, keyboard, context=context)
    else:
...
async def cart_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Повертаємось у кошик для оновлення вигляду
...
async def process_promo(update: Update, context: ContextTypes.DEFAULT_TYPE, silent=False):
    ...
    # Допоміжна функція для безпечного додавання VIP днів
    def extend_vip(days):
        current_expiry_str = profile.get("vip_expiry")
        if current_expiry_str and current_expiry_str != '—':
            try:
    ...
    if text == "GHST2026":
        if profile.get('promo_GHST2026_used'):
            ...
            extend_vip(7) # Додаємо VIP на 7 днів
            
            msg = (
                "✅ <b>GHST2026 УСПІШНО АКТИВОВАНО!</b>\n"
                "━━━━━━━━━━━━━━━━━━━━\n"
                "🎁 <b>Бонус:</b> +69 грн на баланс!\n"
                "💎 <b>Статус:</b> VIP PRO на 7 днів!\n"
                f"📅 Діє до: <b>{profile['vip_expiry']}</b>\n"
                "<i>Використовуй бонус як знижку при наступному замовленні.</i>"
            )
            is_success = True
    ...
    # --- 2. РЕФЕРАЛЬНИЙ КОД (GHST12345) ---
        ...
        if target_id == user.id:
            ...
            if not referrer:
                ...
                try:
                    # Розумне додавання VIP для реферера напряму в БД
                    ref_data = cursor.execute("SELECT vip_expiry FROM users WHERE user_id = ?", (target_id,)).fetchone()
                    ref_expiry_str = ref_data[0] if ref_data else None
                    
                    if ref_expiry_str and ref_expiry_str != '—':
                        try:
                            ref_date = datetime.strptime(ref_expiry_str, "%Y-%m-%d")
                            if ref_date < datetime.now(): ref_date = datetime.now()
                        except: ref_date = datetime.now()
                    else:
                        ref_date = datetime.now()
                        
                    new_ref_expiry = (ref_date + timedelta(days=7)).strftime("%Y-%m-%d")
                    ...
                    await context.bot.send_message(
                        ...
                        text=(
                            ...
                            f"💎 Твій VIP статус продовжено до <b>{new_ref_expiry}</b>.\n\n"
...
# =================================================================
# 📝 SECTION 17: DATA INPUT HANDLER (ULTIMATE SYNC)
# =================================================================
...
async def handle_user_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Головний диспетчер текстових повідомлень.
    Розподіляє ввід між анкетою, промокодами та іншими станами.
    """
    ...
    # 1. Пріоритет: Якщо юзер вводить промокод (навіть поза анкетою)
    if user_text.upper().startswith("GHST"):
        # Якщо юзер НЕ в процесі заповнення важливих даних (адреси), обробляємо промо
        if state not in ['AWAITING_NAME', 'AWAITING_PHONE', 'AWAITING_ADDRESS']:
            if 'process_promo' in globals():
                await process_promo(update, context)
                return

    # 2. Обробка анкети (Data Input Flow)
    # Мапимо старі data_step на нову систему state для сумісності
    if state in ['AWAITING_NAME', 'AWAITING_PHONE', 'AWAITING_ADDRESS'] or context.user_data.get('data_step'):
        await handle_data_input(update, context)
        return

    # 3. Дефолтна відповідь, якщо стан невідомий
    await update.message.reply_text(
        "👋 <b>Привіт!</b> Скористайся меню нижче або введи <code>/start</code>, щоб почати.",
        parse_mode='HTML'
    )

async def handle_data_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Обробляє анкети (Ім'я, Телефон, Адреса).
    Миттєве збереження + Розумна валідація.
    """
    user = update.effective_user
    text = update.message.text.strip()
    
    # Визначаємо поточний крок (підтримка обох ключів для надійності)
    step = context.user_data.get('state') or context.user_data.get('data_step')
    profile = context.user_data.setdefault('profile', {'uid': user.id})

    def save_to_db(field, val):
        try:
            with sqlite3.connect(DB_PATH, timeout=30) as conn:
                # Переконуємось, що юзер є
                conn.execute("""
                ...
                conn.execute(f"UPDATE users SET {field}=? WHERE user_id=?", (val, user.id))
                conn.commit()
            # Синхронізуємо memory-об'єкт
            profile[field] = val
        except Exception as e:
            logger.error(f"DB Direct Save Error ({field}): {e}")

    # --- 1. ІМ'Я ---
    if step == 'AWAITING_NAME' or step == 'name':
        if len(text) < 2 or text.isdigit():
            await update.message.reply_text("⚠️ Введіть коректне Ім'я та Прізвище (тільки літери):")
            return
        
        save_to_db("full_name", text)
        ...
        await update.message.reply_text(
            ...
            f"(Наприклад: <code>0991234567</code>):",
            parse_mode='HTML'
        )

    # --- 2. ТЕЛЕФОН ---
    elif step == 'AWAITING_PHONE' or step == 'phone':
        clean_phone = re.sub(r'[\s\(\)\-\+]', '', text)
        if not clean_phone.isdigit() or len(clean_phone) < 10 or len(clean_phone) > 13:
            await update.message.reply_text("⚠️ Невірний формат. Напишіть номер цифрами (напр. 0931112233):")
            return
        
        save_to_db("phone", clean_phone)
        
        # Перевіряємо місто перед тим як питати адресу
        current_city = profile.get('city')
        if not current_city or current_city == "Не обрано":
            await choose_city_menu(update, context)
        else:
            ...
            await update.message.reply_text(
                f"✅ Номер збережено.\n📍 Ваше місто: <b>{current_city}</b>\n\n"
                f"🏠 Вкажіть <b>Адресу</b> (вулиця, будинок) або <b>№ Відділення НП</b>:",
                reply_markup=InlineKeyboardMarkup(kb),
                parse_mode='HTML'
            )

    # --- 3. АДРЕСА (ФІНАЛ) ---
    elif step == 'AWAITING_ADDRESS' or step == 'address':
        if len(text) < 3:
            await update.message.reply_text("⚠️ Будь ласка, вкажіть повну адресу:")
            return
        
        # Формуємо повну адресу з урахуванням району
        district = profile.get('district')
        full_address = f"{district}, {text}" if district and district not in text else text
        
        save_to_db("address_details", full_address)
        
        # Повністю очищаємо стан, щоб бот більше не чекав тексту
        context.user_data['state'] = None
        context.user_data['data_step'] = None
        ...
        if 'finalize_data_collection' in globals():
            ...
            await update.message.reply_text("✅ Дані успішно збережені в профілі!", 
                                            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("👤 В Профіль", callback_data="menu_profile")]]))
