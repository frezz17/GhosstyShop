...
async def dispatcher(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "catalog":
        ...
        for v in VAPES:
            ...
            btns.append([InlineKeyboardButton(f"{v['name']} | {sale}₴ 🔥", callback_data=f"v_{v['id']}")])
        btns.append([InlineKeyboardButton("🏠 На головну", callback_data="start")])
        await render("🔥 *Каталог вейпів (Ціни зі знижкою 30%):*", WELCOME_PHOTO, InlineKeyboardMarkup(btns))
        ...
        caption = (
            ...
            f"✅ *Ціна за промокодом: {sale} грн*\n"
            f"🚚 Доставка: *Безкоштовна*\n\n"
            f"🎫 Промокод `{ud.get('promo')}` застосовано!\n"
            f"📍 Поточна локація: {loc}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"Знижка діє на перше замовлення."
        )
