...
BOT_USERNAME = "GhostyShopBot" # Замініть на юзернейм вашого бота без @
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and len(args) > 0:
        ...
        if str(ref_id) != str(user.id) and context.user_data.get("referrer") is None:
            ...
            context.user_data["vip"] = True # Даємо VIP тому хто перейшов (як заохочення)
            # В реальному проекті тут треба було б нарахувати бонус тому, чий це ID
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Checkout ---
        ...
        txt = "📜 *Умови, правила, відповідальність*\n\n"
