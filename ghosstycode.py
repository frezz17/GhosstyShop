...
VAPES = [
    {"id": 0, "name": "🍊 Packwoods Orange", "old": 499.0, "img": "https://i.ibb.co/V03f2yYF/Ghost-Vape-1.jpg"},
    {"id": 1, "name": "🌸 Packwoods Pink", "old": 569.0, "img": "https://i.ibb.co/65j1901/Ghost-Vape-2.jpg"},
    {"id": 2, "name": "🍇 Packwoods Purple", "old": 654.0, "img": "https://i.ibb.co/svXqXPgL/Ghost-Vape-3.jpg"},
    {"id": 3, "name": "❄️ Whole Mint", "old": 789.0, "img": 
"https://i.ibb.co/675LQrNB/Ghost-Vape-4.jpg"},
    {"id": 4, "name": "🌴 Jungle Boys", "old": 939.0, "img": "https://i.ibb.co/Zzk29HMy/Ghost-Vape-5.jpg"}
]

NEW_TERMS = (
    "📜 *Умови, правила, відповідальність*\n\n"
    "1️⃣ Проєкт має навчально-демонстраційну ідею магазину, згенерованому ШІ.\n"
    "2️⃣ Інформація подається виключно з ознайомчою метою та всі придумані товарі не існують на території України.\n"
    "3️⃣ Матеріали не є рекомендацією до придбання чи використання одноразових ННС-Вейпів\n"
    "4️⃣ Користувач самостійно несе відповідальність за свої дії, автоматично погоджуючиссь з усіма положеннями цієї угоди користувача.\n"
    "5️⃣ Адміністрація не зберігає персональні дані користуваів бота та не використовує їх.\n"
    "6️⃣ Розробник не несе ні якої відповідальності за дії користувача у боті або при придбанні платної услуги зі списку пристроїв з ціною за персональню знижкою -35% у формі послуги Спілкування з менеджером.\n\n"
    "⚠️ *Важливо:*\n"
    "7️⃣ Магазин не є реальним та не здійснює продаж товарів на території України.\n"
    "8️⃣ Жоден товар не буде доставлений користувачу, магазин не має існуючих складів на території України.\n"
    "9️⃣ Усі переказані кошти вважаються добровільним подарунком розробнику та кодеру та не можуть бути повернуті.\n"
...
def get_main_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🏙 Обрати місто 📍", callback_data="loc_cities")],
...
async def send_welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Ваш промокод на знижку *-30%*: `{context.user_data['promo']}`\n"
        f"📅 Дійсний до: *{PROMO_EXPIRY}*\n\n"
        f"Оберіть пункт меню нижче: 👇"
    )
...
query = update.callback_query
    ...
    if data == "catalog":
        btns = []
        for v in VAPES:
            new_price = round(v['old'] * DISCOUNT_RATE, 2)
            btns.append([InlineKeyboardButton(f"{v['name']} | ~{v['old']}~ → {new_price}₴ 🔥", callback_data=f"v_{v['id']}")])
        btns.append([InlineKeyboardButton("🏠 На головну", callback_data="start")])
        await render("🔥 *Каталог вейпів (Знижка 35%):*", WELCOME_PHOTO, InlineKeyboardMarkup(btns))

    elif data.startswith("v_"):
        v = VAPES[int(data.split("_")[1])]
        new_price = round(v['old'] * DISCOUNT_RATE, 2)
        loc = f"{ud.get('city','❌')}, {ud.get('dist','❌')}"
caption = (
            f"*{v['name']}* 🌿\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"❌ Стара ціна: ~{v['old']}~ грн\n"
            f"✅ Нова ціна зі знижкою 35%: {new_price} грн\n"
            f"🚚 Доставка: *Безкоштовна*\n\n"
            f"📍 Поточна локація: {loc}\n"
            f"━━━━━━━━━━━━━━━━━━━━"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🛍 Замовити у менеджера", url=MANAGER_URL)],
            [InlineKeyboardButton("🏙 Змінити локацію", callback_data="loc_cities")],
            [InlineKeyboardButton("⬅️ Назад до списку", callback_data="catalog")]
        ])
    ...
    # --- LOCATION LOGIC ---
        ...
        for c in LOCATIONS.keys():
            mark = "✅ " if ud.get("city") == c else ""
            btns.append([InlineKeyboardButton(f"{mark}{c}", callback_data=f"sc_{c}")])
        btns.append([InlineKeyboardButton("🏠 На головну", callback_data="start")])
        await render("🏙 *Оберіть ваше місто:*", WELCOME_PHOTO, InlineKeyboardMarkup(btns))
        ...
        for d in LOCATIONS[city]:
            mark = "✅ " if ud.get("dist") == d else ""
            btns.append([InlineKeyboardButton(f"{mark}{d}", callback_data=f"sd_{d}")])
        btns.append([InlineKeyboardButton("⬅️ До списку міст", callback_data="loc_cities")])
        await render(f"🏘 *{city}*. Оберіть район:", WELCOME_PHOTO, InlineKeyboardMarkup(btns))

    elif data.startswith("sd_"):
        ud["dist"] = data.replace("sd_", "")
        await render(f"✅ *Локацію збережено!*\n\n📍 {ud['city']}\n🏘 {ud['dist']}", WELCOME_PHOTO, get_main_kb())
...
photos = await context.bot.get_user_profile_photos(user.id)
        ...
        profile_text = (
            ...
            f"🆔 *Ваш ID:* `{user.id}`\n"
            f"📅 *Реєстрація:* {ud.get('reg_date')}\n\n"
            f"🎫 *Промокод:* `{ud.get('promo')}`\n"
            f"🔥 *Знижка:* -30% на 1-ше замовлення\n"
            f"⏳ *Термін дії:* до {PROMO_EXPIRY}\n\n"
            f"📍 *Місто:* {ud.get('city','❌ Не обрано')}\n"
            f"🏘 *Район:* {ud.get('dist','❌ Не обрано')}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"💡 _Натисніть на ID або Код для копіювання_"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🏙 Обрати місто/район", callback_data="loc_cities")],
...
await render(profile_text, p_img, kb)
    ...
    # --- TERMS & HOME ---
        ...
        await render(NEW_TERMS, WELCOME_PHOTO, InlineKeyboardMarkup([[InlineKeyboardButton("🏠 На головну", callback_data="start")]]))
...
def main():
    ...
    print("🚀 System Online | Ghossty Master Engine")
